...
function MediaPostImage({ image, isActive, onClick }: { image: TMediaPost, isActive: boolean, onClick: () => void }) {
	return (
		<div className={`img 
		...
		`} key={image.id}>
			...
			{/* <div className="add-media absolute left-1 top-1 bg-[#D9D9D9] hover:bg-gray-300 p-2 rounded-md cursor-pointer">
				<div className="add-media-button flex gap-2 items-center">
					<svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path d="M7.125 1V13.25M1 7.125H13.25" stroke="#828282" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
					</svg>
			...
			<img
				src={image.url}
				className="max-h-[260px] min-[2000px]:max-h-[300px] w-full rounded-md object-cover"
			/>
		</div>
	)
}

export default function MediaPost({ extendedClass }: { extendedClass: string }) {
	...
	return (
		...
		`}>
			...
			">
				{ media.map((m, index) => {
					return <MediaPostImage image={m} isActive={index === activeIndex} onClick={() => removeMedia(m.id, index === media.length - 1)} />
				})}
			</div>
